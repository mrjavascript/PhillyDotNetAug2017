...
function ToastMessage(props) {
    if (props.message == 'Online')
        return <span className="toast" id="toast-message"
                     style={{opacity: 1}}><SvgWifi/>&nbsp;&nbsp;{props.message}&nbsp;</span>;
    else if (props.message == 'Offline')
        return <span className="toast" id="toast-message"
                     style={{opacity: 1}}><SvgWifiOff/>&nbsp;&nbsp;{props.message}&nbsp;</span>;
    else
        return <span className="toast" id="toast-message"
                     style={{opacity: 1}}><SvgCheck/>&nbsp;{props.message}&nbsp;</span>;
}
...
function SvgMail(props) {
    return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="#424242" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function SvgCheck(props) {
    return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="#1de9b6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
...
function SvgWifiOff(props) {
    return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="#ff1744" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function UserCard(props) {
    ...
    return <div className="card fluid col-sm-12" id={"user_" + user.login.username}>
        ...
        <div className="section">
            <p className="user-birth"><SvgCalendar/>&nbsp;&nbsp;{user.dob.split(" ")[0].split("-").reverse().join("/")}
            </p>
        </div>
        <div className="section">
...
function renderPosts() {
    var postCards = posts.map(
        function (post, key) {
            return <PostCard text={post} userId={Math.floor(Math.random() * 25)}
                             image={imagesEndpoint + randomImageId()} key={key}/>;
        }
    );
    ReactDOM.render(
